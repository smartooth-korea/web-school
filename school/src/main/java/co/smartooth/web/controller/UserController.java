...
public class UserController {
	...
	@Autowired(required = false)
	private TeethService webTeethService;

	@Autowired(required = false)
	private OrganService webOrganService;

	@Autowired(required = false)
	private LocationService webLocationService;
	 ...
	 * 기능 : 개인정보 동의서로 회원가입
	 */
	@PostMapping(value = { "/school/user/insertUserInfo.do" })
	public String insertUserInfo(@RequestParam Map<String, String> paramMap, HttpServletRequest request, HttpSession session, Model model, RedirectAttributes redirectAttributes) throws Exception {
		...
		organVO = webOrganService.selectOrganInfo(schoolCode);
		// 주소 정보 : 시도
		...
		// 나라 코드로 국가명 조회
		locationVO = webLocationService.selectNationalInfo(countryCd);
		...
		// 피측정자 회원 정보 VO
		stUserVO.setUserId(userId);
		stUserVO.setUserName(userName);
		stUserVO.setUserPwd(userPwd);
		stUserVO.setUserType("ST");
		stUserVO.setUserBirthday(userBritday);
		stUserVO.setUserCountry(countryCd);
		stUserVO.setUserTelNo(paUserTelNo);
		stUserVO.setUserSex(userSex);
		stUserVO.setCountryNm(countryNm);
		stUserVO.setSidoNm(organSidoNm);
		stUserVO.setSigunguNm(organSigunguNm);
		stUserVO.setEupmyeondongNm(organEupmyeondongNm);
		stUserVO.setAddrDetail(addressDetail);
		
		// 피측정자 회원 치아 상태 등록
		teethInfoVO.setUserId(userId);
		teethInfoVO.setTeethStatus(userTeethStatus);
		
		// 법정대리인 회원 아이디 :: 피측정자 이름+생년월일(190101 :: stTmpUserPwd)+전화번호 뒷자리4자리
		...
		// 법정대리인 회원 정보 VO
		paUserVO.setUserId(prUserId);
		paUserVO.setUserName(prUserName);
		paUserVO.setUserPwd(paUserPwd);
		paUserVO.setUserType("PR");
		paUserVO.setUserTelNo(paUserTelNo);
		paUserVO.setUserCountry(countryCd);
		paUserVO.setCountryNm(countryNm);
		paUserVO.setSidoNm(organSidoNm);
		paUserVO.setSigunguNm(organSigunguNm);
		paUserVO.setEupmyeondongNm(organEupmyeondongNm);
		paUserVO.setAddrDetail(addressDetail);
		...
		webUserService.insertUserInfo(stUserVO);
		...
		webUserService.insertUserInfo(paUserVO);
		// 계정 상세 정보 등록 (법정대리인) - ST_PARENT_USER_DETAL
		webUserService.insertPaUserDetail(prUserId, userId);
		// 치아 상태
		webTeethService.insertUserTeethInfo(teethInfoVO);
		// 피측정자 회원 시퀀스 증가
		webOrganService.updateOrganUserSeqNo(organVO);
...
//		// 법정대리인
//		UserVO webPrUserVO = new UserVO();
//		// 기관 정보
//		OrganVO webOrganVO = new OrganVO();
//		// 지역 정보 (나라)
//		LocationVO webLocationVO = new LocationVO();
//		// 치아 상태 정보
//		TeethInfoVO webTeethInfoVO = new TeethInfoVO();
//
//		// 암호화 클래스
//		AES256Util aes256Util = new AES256Util();
//		
...
//		
//		// 만 14세 미만일 경우 : 법정 대리인, 피측정자의 정보 입력
...
//			
//			// 법정대리인 회원 아이디 :: 피측정자이름+생년월일(190101 :: stTmpUserPwd)+전화번호 뒷자리4자리
//			prUserId = userName+strBirthday+paUserTelNo3;
//			
//			// 법정대리인 회원 비밀번호 :: 휴대전화 번호 뒷자리 4자리 
//			paUserPwd = aes256Util.aesEncode(paUserTelNo3);
