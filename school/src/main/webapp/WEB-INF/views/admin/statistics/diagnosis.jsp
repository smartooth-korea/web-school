...
<link rel="stylesheet" href="/css/admin/statistics/diagnosis.css">
...
var cavityNormal = "${cavityNormal}";  
var cavityCaution = "${cavityCaution}";
var cavityDanger = "${cavityDanger}";  
...
$(document).ready(function() {
	...
	// container 높이 변경 (5px)
	$("#container").css("height", containerHeight + variableHeight + 25);
	// comment 높이 변경 (5px)
	$("#comment").css("height", commentHeight + variableHeight + 10); // 정답
	// contents 높이 변경 (5px)
	$("#contents").css("height", contentsHeight + variableHeight + 10);
    ...
    $("#btn_editDescript").click(function(e){
		...
		$.ajax({
			...
			url:'/admin/statistics/updateDiagDescript.do',   //데이터를 주고받을 파일 주소
    ...
    $("#btn_editMemo").click(function(e){
		...
		$.ajax({
			type:'POST',   //post 방식으로 전송
			url:'/admin/statistics/updateMemo.do',   //데이터를 주고받을 파일 주소
			data:JSON.stringify ({ //변수에 담긴 데이터를 전송해준다 (JSON 방식)
				"userId" : userId
				,"measureDt" : measureDt
